...
class ControllerRos(Controller):
    def __init__(self):
        ...
        self._prev_joy = None
        self._nopress_index = 0
        self._do_disconect = 0
        self._need_disconect = False
    ...
    def cb_report(self, report):
        ...
        if self.use_standard_msgs:
            ...
            button_sum = 0
            for c in joy_msg.buttons:
                button_sum = button_sum + c
                #print("button_sum %d" % button_sum)

            if button_sum > 0 or (self._prev_joy != None and (joy_msg.buttons != self._prev_joy.buttons or joy_msg.axes != self._prev_joy.axes) ):
                if button_sum >= 1 \
            ...
            if self._prev_joy != None :
                self._prev_joy.header = joy_msg.header
            else:
                self._prev_joy = joy_msg

            if not self._need_disconect:
                self._prev_joy = joy_msg
    ...
    def cb_joy_pub_timer(self, _):
        lock.acquire()
        self._nopress_index = self._nopress_index + 1
        if self._need_disconect:
            self._do_disconect = self._do_disconect + 1
            if self._nopress_index > self._autorepeat_rate * 60 *5 :
                if self._prev_joy != None and self._do_disconect > self._autorepeat_rate * 60:
                    print("###### Joystick inactive for 300 seconds. Disconnecting to save battery  ###### ")
                    os.system("bluetoothctl disconnect")
                    self._do_disconect = 0
                self._prev_joy = None
        else:
            self._do_disconect = 0
            if self._nopress_index > self._autorepeat_rate * 3 :
                self._need_disconect = True
                print("###### Joystick inactive for 3 seconds. Stop publishing joy")
                if self._prev_joy != None:
                    self._prev_joy.axes = [
                        0.0,
                        0.0,
